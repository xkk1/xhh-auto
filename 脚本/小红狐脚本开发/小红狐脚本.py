...
页面操作: dict[str, dict[str, Any]] = {
    ...
    "全屏鼠标坐标监听": {  # 页面操作脚本名，唯一
        "入口函数": 全屏鼠标坐标监听.入口函数,  # 入口函数 (页面名: str)
        "关闭函数": 全屏鼠标坐标监听.关闭函数,  # 关闭函数 (页面名: str | None) 页面名=None->关闭全部
        "脚本状态": 全屏鼠标坐标监听.脚本状态,  # 脚本开启状态函数 (页面名: str | None) -> bool
        "配置页面": None,  # 配置页面函数 (页面名: str) -> dict[str, str]
        "名称": "全屏鼠标坐标监听",  # 脚本名称 str
        "简介": "监听并在控制台显示全屏鼠标坐标，支持点击和左键拖动",  # 脚本简介 str
        "调试": True,  # 仅开启调试模式时启用脚本 bool 默认：False
    },
}
