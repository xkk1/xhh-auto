...
class 页面类:
    ...
    def 关闭(self, 原因: Union[str, None]=None, 卸载前执行: Union[bool, None]=None) -> None:
        """关闭页面

        关闭当前页面

        Args:
            原因 (Union[str, None], optional): 要报告给因页面关闭而中断的作的原因。
            卸载前执行 (Union[bool, None], optional): 默认为 false, 不运行任何卸载处理程序并等待页面关闭，默认情况下不会在卸载处理程序之前运行。
    ...
    def 截图(
            self,
            区域: Optional[Dict[str, float]] = None,
            缩放: Optional[Literal['css', '设备']]=None,
            *args, **kwargs
    ) -> bytes:
        """截图
        ...
        Args:
            区域 (Optional[Dict[str, float]], optional): 可选。指定截图的矩形区域，格式为：
                ```
                {
                    "x": float,    # 起始 X 坐标
                    "y": float,    # 起始 Y 坐标
                ...
                ```
                如果为 None，则截取整个视口
            缩放 (Optional[Literal['css', '设备']], optional): 可选。指定截图的缩放级别。
                - "css"：CSS 缩放级别（默认）
                - "设备"：设备缩放级别
    ...
    def 添加初始化脚本(self, 路径: Union[str, pathlib.Path]=None, 脚本: str=None) -> None:
        """添加初始化脚本
        ...
        Args:
            路径 (Union[str, pathlib.Path], optional): JavaScript 文件的路径。如果 path 是相对路径，则相对于当前工作目录进行解析。
            脚本 (str, optional): 要在浏览器上下文中的所有页面中评估的脚本。
...
class 鼠标类:
    ...
    def 点击(self, x: float, y: float, 按钮: Literal['左', '中', '右']="左", 点击次数: int=1, 延迟: float=0) -> None:
        """鼠标点击

        Args:
            x (float): X 相对于主框架视口的坐标（以 CSS 像素为单位）。
            y (int): Y 相对于主框架视口的坐标（以 CSS 像素为单位）。
            按钮 (Literal['左', '中', '右'], optional): 鼠标按钮，默认为 "左"。
            点击次数 (int, optional): 点击次数，默认为 1。
            延迟 (float, optional): 按下鼠标和鼠标向上之间的等待时间（以毫秒为单位）。默认为 0。
    ...
    def 按下(self, 按钮: Literal['左', '中', '右']="左", 点击次数: int=1) -> None:
        """鼠标按下
    ...
    def 抬起(self, 按钮: Literal['左', '中', '右']="左", 点击次数: int=1) -> None:
        """鼠标抬起
    ...
    def 移动(self, x: float, y: float, 步数: int=1) -> None:
        """鼠标移动

        Args:
            x (float): X 相对于主框架视口的坐标（以 CSS 像素为单位）。
            y (int): Y 相对于主框架视口的坐标（以 CSS 像素为单位）。
            步数 (int, optional): 默认为 1，发送中间鼠标移动事件。
    ...
    def 双击(self, x: float, y: float, 按钮: Literal['左', '中', '右']="左", 延迟: float=0) -> None:
        """鼠标双击
    ...
    def _鼠标按钮转换英文(self, 按钮: Literal['左', '中', '右']="左") -> Literal['left', 'middle', 'right']:
        """鼠标转换按钮
        ...
        Returns:
            Literal['left', 'middle', 'right']: 鼠标按钮
        """
        return "left" if 按钮 == "左" else "middle" if 按钮 == "中" else "right"
...
class 键盘类:
    ...
    def 输入(self, 文本: str, 延迟: float=0) -> None:
        ...
        Args:
            文本 (str): 要键入焦点元素的文本。
            延迟 (float, optional): 按键之间的等待时间（以毫秒为单位）。默认为 0。
