...
class 数据类:
    def __init__(self, 默认值: Optional[Dict[str, Any]] = None, 路径: Optional[Union[pathlib.Path, str]]=None):
        self.默认值 = 默认值
    ...
    def __setitem__(self, 键, 值):
        # 设置值
        self.数据[键] = 值
    
    def __delitem__(self, 键):
        # 删除键
        del self.数据[键]
    
    def __contains__(self, 键):
        # 判断键是否存在
        return 键 in self.数据
...
def 获取本地数据(路径: Union[pathlib.Path, str], 默认值: Optional[Dict[str, Any]] = None) -> 数据类:
    """获取本地数据"""
    路径 = pathlib.Path(路径)
...
def 获取内存数据(标识, 默认值: Optional[Dict[str, Any]] = None) -> 数据类:
    """获取内存数据"""
    if 标识 in 内存数据字典:
        return 内存数据字典[标识]
