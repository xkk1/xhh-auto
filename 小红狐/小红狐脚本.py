...
页面生成: dict[str, dict[str, Any]] = {
    "小红狐页面生成": {  # 页面生成脚本名，唯一
        "入口函数": None,  # 入口函数 (页面名: str) -> dict[str, Any] 至少包含"页面"
        "关闭函数": None,  # 关闭函数 (页面名: str | None)
        "页面状态": None,  # 页面开启状态函数 (页面名: str) -> bool
...
from .内置脚本.页面操作 import 小红狐页面管理

页面操作: dict[str, dict[str, Any]] = {
    "小红狐页面管理": {  # 页面操作脚本名，唯一
        "入口函数": 小红狐页面管理.入口函数,  # 入口函数 (页面名: str)
        "关闭函数": 小红狐页面管理.关闭函数,  # 关闭函数 (页面名: str | None) None 关闭全部
        "脚本状态": 小红狐页面管理.脚本状态,  # 脚本开启状态函数 (页面名: str | None) -> bool
        "配置页面": 小红狐页面管理.配置页面,  # 配置页面函数 (页面名: str) -> dict[str, str]
        "简介": "小红狐页面管理",  # 脚本简介
        "调试": False,  # 仅开启调试模式时启用脚本。默认：False
    },
    "小红狐脚本开发者工具": {  # 页面操作脚本名，唯一
        "入口函数": None,  # 入口函数
        "关闭函数": None,  # 关闭函数
...
默认启用页面操作脚本名元组: tuple[str, ...] = ("小红狐页面管理",)
